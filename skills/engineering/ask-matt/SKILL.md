...
- **`/humanizer-ru`** — the Russian-prose editor. It runs **audit, fix, verify** as three phases that never overlap — a model that rewrites while it reads repeats its own tells and misses half the list — and closes on a deterministic linter. Reach for it on Russian copy a **human** will read: sales scripts, comments, site copy, an AI draft about to go out under your name. It also runs **detect-only**: a verdict with quoted evidence and not one word changed. Model-invoked. Not `/wait-what`, which re-pitches what the **agent** just said rather than editing a text of yours; not `/writing-for-agents`, which is about documents **agents** read.
