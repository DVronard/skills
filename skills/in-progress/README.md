...
- **[unlazy](./unlazy/SKILL.md)** — Enforce completion discipline on substantial work by writing runnable acceptance gates before execution, decomposing with the Depth Tree, and reverifying evidence before reporting done. Vendored from [Leonxlnx/unlazy](https://github.com/Leonxlnx/unlazy) (MIT).
- **[ci-minutes](./ci-minutes/SKILL.md)** — Cut GitHub Actions minutes on a Free-plan account: measure the per-run cost first, then path-filtered workflows per side, one test run with a precise skip guard, and no push-to-main run — each with its price named, gated and mutation-checked. Instructions in Russian. User-invoked.
