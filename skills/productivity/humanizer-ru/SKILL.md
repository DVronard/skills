...
name: humanizer-ru
license: MIT
description: Редактор русского текста - убирает признаки AI-генерации, канцелярит и воду, сохраняя факты. Use when пользователь просит «отредактируй», «убери воду», «сделай живым», «перепиши человечнее», «humanize», «очисти от AI», говорит что текст «звучит как нейросеть», готовит AI-черновик к публикации, или спрашивает «проверь на AI», «палится ли текст», «это писала нейросеть?» (detect-режим - аудит без правки). Do NOT use for текста с нуля, перевода, орфографии и пунктуации, нерусского текста, кода и конфигов, юридических и академических текстов.
---
...
8. **Запусти линтер по чистовику** (без changelog и цитат «до»). Путь отсчитывается от папки этого скилла, как и `references/patterns.md`; из другой рабочей директории подставь её абсолютный путь целиком:
   ```bash
   ...
   - Скрипт не найден - это не отказ гейта, а неверный путь: найди папку этого скилла (рядом с ней лежат `references/` и `scripts/`) и повтори запуск по абсолютному пути.
   - Питона нет - проверь чеклист «Жёсткие запреты» руками, каждую строку, и назови в резюме линт непрогнанным.
...
Портировано из [humanizer-ru](https://github.com/smixs/humanizer-ru) v1.7 (коммит `ff990df`), автор Serge Shima, лицензия MIT - полный текст в `LICENSE` рядом.
