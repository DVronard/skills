...
Before starting work on any task in this repository, invoke the [`unlazy`](./skills/in-progress/unlazy/SKILL.md) skill first: write the acceptance gates (`GATES.md`), work the task through its Depth Tree, and reverify evidence before reporting anything done. This applies whether the task is adding a skill, editing docs, touching a manifest, or fixing a bug in one of the scripts.
